...
#database
conn=sqlite3.connect('rodrigo.db')
c=conn.cursor()


def submit(): 
    conn=sqlite3.connect('C:/Users/STUDENTS/rodrigo.db')
    c=conn.cursor()
...
def query():
    conn=sqlite3.connect('C:/Users/STUDENTS/rodrigo.db')
    c=conn.cursor()
    c.execute("SELECT *,oid FROM studentinfo")
    records=c.fetchall()
...
def delete():
    conn=sqlite3.connect('C:/Users/STUDENTS/rodrigo.db')
    c=conn.cursor()
    c.execute("DELETE from studentinfo WHERE oid="+delete_box.get())
...
def edit():
    ...
    conn.close()
'''
c.execute ("""CREATE TABLE "studentsinfo"(
          "f_name" TEXT,
          "l_name" TEXT,
          "age" INTEGER,
          "address" TEXT,
